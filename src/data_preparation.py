...
def feature_extraction(dset):
    ...
    ds.loc[:, 'prop_review_score'] = dset['prop_review_score'].fillna(0).astype(float)
    ds.loc[:, 'loc_ratio2'] = loc_ratio2(dset).fillna(0).astype(float)
    ds.loc[:, 'norm_star_rating'] = norm_pcid(dset, 'prop_starrating').astype(float)
    ds.loc[:, 'nlog_price'] = log_norm_srch_id(dset, 'price_usd').fillna(0).astype(float)
    ds.loc[:, 'label'] = label(dset).astype(float)
